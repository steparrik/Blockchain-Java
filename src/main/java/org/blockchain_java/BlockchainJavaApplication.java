...
public class BlockchainJavaApplication {
	...
	public static void main(String[] args) {
		...
		LevelDBService levelDBService = context.getBean(LevelDBService.class);
		Gson gson = context.getBean(Gson.class);

		levelDBService.put("stepa:0", gson.toJson(new TransactionOutput("stepa", new BigDecimal(1000), false)));
		System.out.println(levelDBService.get("stepa:0"));
		while (true){
			String from = sc.next();
			String to = sc.next();
